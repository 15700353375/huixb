...
import { initRecentContactList, initUnreadMsgCount } from './recent_contact_list_manager'
...
export { addMsg, convertMsgtoHtml}
...
function addMsg(msg,prepend) {
    ...
    let infoMap = app.$store.state.chat.infoMap;
    ...
    // 解析消息
    msg.message = convertMsgtoHtml(msg)
    ...
    switch (subType) {
        ...
        case webim.GROUP_MSG_SUB_TYPE.REDPACKET://群红包消息
            msg.message = "[群红包消息]" + convertMsgtoHtml(msg);
            break;
        case webim.GROUP_MSG_SUB_TYPE.LOVEMSG://群点赞消息
            //业务自己可以增加逻辑，比如展示点赞动画效果
            msg.message = "[群点赞消息]" + convertMsgtoHtml(msg);
            //展示点赞动画
            //showLoveMsgAnimation();
            break;
        case webim.GROUP_MSG_SUB_TYPE.TIP://群提示消息
            msg.message = "[群提示消息]" + convertMsgtoHtml(msg);
            break;
    }
    ...
    let isOpen = false;
    // 当前聊天对象窗口打开状态
    if(app.$store.state.chat.selToID == msg.fromAccountNick){
        isOpen = true;
    }
    console.log(app.$store.state.chat.MsgList);
    

 
    
    
    // 此时我们将最近聊天内容添加到最近联系人中--没有刷新最近联系人。可以打开窗口的时候刷新一下最近联系人、
    let recentSessMap = app.$store.state.chat.recentSessMap
    let ind = _.findKey(recentSessMap, item => { return item.SessionId == msg.fromAccountNick });
        if(ind){
            if(!isOpen){
               recentSessMap[ind].UnreadMsgCount++;                
            } 
            recentSessMap[ind].MsgShow = msg.message;
            recentSessMap[ind].MsgShow = checkMsgType(msg);
            recentSessMap[ind].MsgTimeStamp = msg.time;
            if(isOpen){
                app.$store.state.chat.MsgList.push(msg);
                // recentSessMap[ind].MsgShow = checkMsgType(msg);
                // 已读上报
    ...
    if(msg.isSend){
        ...
        let ind = _.findKey(recentSessMap, item => { return item.SessionId == app.$store.state.chat.selToID });
        if(ind){      
    ...
    for(var j in recentSessMap){   //variable 为属性名
        console.log(j)
        if(recentSessMap[j].UnreadMsgCount > 0){
            app.$store.state.chat.messageCount += recentSessMap[j].UnreadMsgCount
        }
    }
    app.$store.commit('chat/setRecentSessMap', recentSessMap);
    ...
    // // 最近消息刷新一下
    // if(deal_history_msg(app.$store.state.chat.selToID)){
    //     
    //     deal_history_msg(app.$store.state.chat.selToID)
    // }
