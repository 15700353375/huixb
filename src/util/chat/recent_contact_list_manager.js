...
var initRecentContactList = function (cbOK, cbErr) {
    ...
    let maxNameLen = app.$store.state.chat.maxNameLen;
    ...
    webim.getRecentContactList(
        ...
        function (resp) {
            ...
            if (resp.SessionItem && resp.SessionItem.length > 0) { // 如果存在最近会话记录
                ...
                // debugger;
                // 最近联系人对象 存进vuex。目前是登录成功之后或者刷新之后执行此操作
                // app.$store.state.chat.recentSessMap = recentSessMap
                
                app.$store.commit('chat/setRecentSessMap', recentSessMap);
                
                // 每次拉取联系人、匹配头像昵称
                common.matchNick(recentSessMap)
    ...
    // app.$store.state.chat.infoMap = infoMap;
};
...
function updateSessDiv(sess_type, to_id, name, unread_msg_count) {
    ...
    if(unread_msg_count){
        ...
        let ind = _.findKey(recentSessMap, item => { return item.SessionId == to_id });
        if(ind){
            recentSessMap[ind].UnreadMsgCount = unread_msg_count;
...
export { initRecentContactList, initUnreadMsgCount}